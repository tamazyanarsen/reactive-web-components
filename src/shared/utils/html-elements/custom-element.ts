...
export const createCustomElement = <T extends BaseElement>(
  ...
  wrapper.init = () => {
     initComponent(component, config);
  }
	return component;
};
...
export const createCustomEl = <T extends BaseElement>(
	...
	return (...content: ChildrenContent<T>[]) => {
		...
		comp.hostElement.slotContent = newContent
			.filter(isComponentConfig)
			.reduce((acc, item) => {
				const currSlotName =
					item.hostElement.getAttribute('slot') || 'default';
				if (!acc[currSlotName]) {
					acc[currSlotName] = [];
				}
...
>(
	...
	return createCustomEl(
		`${srcComp.renderTagName}${
			classList && typeof classList === 'string' ? ' ' + classList : ''
		}`,
