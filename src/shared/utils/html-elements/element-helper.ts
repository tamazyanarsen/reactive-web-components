...
import { observeChildRemoval } from '../clean';
...
>(
	...
	additionalFields
		...
		.forEach((key: string) => {
			...
			config.effects.push(
				config[key as keyof ComponentInitConfig<T>] as any
			);
		});
	...
	// ANS: Если оставить и для обычных, то будет сноситься реактивность после кеширования (только для селектов с запросом) (кривое кеширование? а что если when?)
	// Она и так удаляется из-за удаления из DOM веб-компонента
	// Рассмотреть все случаи, когда див удалён и не будет пересоздан вновь
	// кажется, что всё нормально работает, но элемент делает disconnectCallback и реактивность удаляется
	// if (!(component.hostElement instanceof BaseElement)) {
	const effectValue = effect(useHelpers);
	observeChildRemoval(component.hostElement, () => {
		effectValue.destroy();
	});
	// } else {
	// useHelpers();
	...
	if (attrObject) {
		Object.keys(attrObject).forEach((attrName) => {
			if (isReactiveSignal(attrObject[attrName])) {
				...
				comp.addEffect(() => {
					comp.setCustomAttribute(
						attrName,
						(attrObject[attrName] as any)()
					);
				});
			} else {
...
export const addAttributeList = <T extends ExtraHTMLElement>(
	...
	if (attrObject) {
		Object.keys(attrObject).forEach(
			// @ts-expect-error error
			<K extends AttrSignal<T>>(attrName: K) => {
