...
export class HtmlComponentConfig<T extends ExtraHTMLElement>
	implements ComponentConfig<T>
{
	...
	append: ComponentConfig<T>['append'] = (...args) => {
		args.forEach((element) => {
			...
			if (ON_CONNECTED_NAME in element.hostElement) {
				setTimeout(() => {
					(
						element.hostElement as unknown as ExtraHTMLElement
					).onConnected?.(element, element.hostElement);
				});
			}
		});
		return this;
	...
	setAttribute: ComponentConfig<T>['setAttribute'] = (attrName, value) => {
		...
		if (
			...
			!(this.wrapper instanceof BaseElement)
		) {
			if (value) {
				newValue = '';
			} else {
				...
				if (attrNameLowerCase in this.wrapper) {
					this.wrapper[
						attrNameLowerCase as keyof typeof this.wrapper
					] = null as any;
				}
				return this;
			}
		...
		if (!(this.wrapper instanceof BaseElement)) {
			const attrNameLowerCase = attrName.toLowerCase();
			if (attrNameLowerCase in this.wrapper) {
				this.wrapper[attrNameLowerCase as keyof typeof this.wrapper] =
					value as any;
			}
		}
		return this;
	...
	setReactiveAttribute: ComponentConfig<T>['setReactiveAttribute'] = (
		...
		const effectValue = effect(() =>
			this.setAttribute(attrName, valueSignal())
		);
		if (this.wrapper instanceof BaseElement)
			this.wrapper.destroyDependenciesEffects.push(effectValue.destroy);
		return this;
	};
	setReactiveCustomAttribute: ComponentConfig<T>['setReactiveCustomAttribute'] =
		(attrName, valueSignal) => {
			const effectValue = effect(() =>
				this.setCustomAttribute(attrName, valueSignal())
			);
			if (this.wrapper instanceof BaseElement)
				this.wrapper.destroyDependenciesEffects.push(
					effectValue.destroy
				);
			return this;
		};
	...
	addEffect: ComponentConfig<T>['addEffect'] = (cb) => {
		const effectValue = effect(() => cb(this, this.hostElement));
		if (this.wrapper instanceof BaseElement)
			this.wrapper.destroyDependenciesEffects.push(effectValue.destroy);
		return this;
	};
	...
	implements CustomComponentConfig<T>
{
	setReactiveValue: CustomComponentConfig<T>['setReactiveValue'] = (
		...
		if (this.wrapper instanceof BaseElement)
			this.wrapper.setReactiveValue(value);
		return this;
	};
