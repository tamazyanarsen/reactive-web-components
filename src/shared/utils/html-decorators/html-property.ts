...
export const event =
	...
	(target, propName) => {
		...
		(Reflect.get(target, EVENT_CONFIG) as EventConfigObj)[
			propName as string
		] = {
			bubbles: config?.bubbles ?? false,
			composed: config?.composed ?? false,
		};
		(Reflect.get(target, eventFieldName) as string[]).push(
			propName as string
		);
	};
		...
		class NewClass extends target {
			...
			connectedCallback() {
				...
				const wrapperEffect = () => {
						...
						Object.entries(this.providers).forEach(
							([provider, providerValue]) => {
								...
								this.addEventListener(provider, (e: Event) => {
									...
									if (
										customEvent.detail?.context === provider
									) {
					...
					(
						target.prototype[eventFieldName] as string[] | undefined
					)?.forEach((fieldName) => {
					...
					const insertRenderTemplate = () => {
						...
						const renderComponent =
							this.render() as ComponentConfig<any>;
					...
					if (this.rootStyle && !target.styles) {
						...
						const appendStyle = (css: string) => {
							...
							const propertyCss = new CSSStyleSheet();
							propertyCss.replaceSync(
								css.slice(css.indexOf('@property'))
							);
							document.adoptedStyleSheets.push(propertyCss);
						};

						if (isStylePromise(styleValue)) {
							const stylePromise: Promise<
							...
							Promise.all(stylePromise)
								.then((v) =>
									v.forEach((e) => appendStyle(e.default))
								)
					...
					if (
						this.slotContext &&
						Object.keys(this.slotContext).length > 0
					) {
						this.shadow
							...
							.forEach((slotEl) => {
								...
								slotEl
									...
									.forEach((slotItem) => {
										...
										if (
											this.slotContext &&
											isReactiveSignal(slotContextValue)
										) {
											colorLog(
												'@oslot element',
												slotEl,
				...
				const effectValue = effect(wrapperEffect);
				this.destroyDependenciesEffects.push(effectValue.destroy);
				componentStack.pop();
			}

			disconnectedCallback() {
				this.destroyDependenciesEffects.forEach(effect => effect());
				this.destroyDependenciesEffects = [];
